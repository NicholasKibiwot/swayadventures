...
const tourOptions = [
{ id: 1, title: 'Maasai Mara Great Migration', price: 1200, duration: '7 days', category: 'safari' },
{ id: 2, title: 'Diani Beach Getaway', price: 650, duration: '4 days', category: 'beach' },
{ id: 3, title: 'Mount Kenya Trek', price: 850, duration: '5 days', category: 'retreat' },
{ id: 4, title: 'Amboseli Elephant Safari', price: 750, duration: '3 days', category: 'safari' },
{ id: 5, title: 'Lamu Island Retreat', price: 900, duration: '6 days', category: 'beach' },
{ id: 6, title: 'Tsavo Red Elephant Safari', price: 700, duration: '4 days', category: 'safari' },
{ id: 7, title: 'Naivasha Lake Retreat', price: 450, duration: '3 days', category: 'retreat' },
{ id: 8, title: 'Samburu Rare Species Safari', price: 980, duration: '5 days', category: 'safari' }];
...
export default function BookingClientPage() {
  ...
  const selectTour = (tour: typeof tourOptions[0]) => {
    setForm((prev) => ({
      ...prev,
      tourId: tour.id,
      tourName: tour.title,
  ...
  // When user tries to go from step 2 → 3, require auth
  ...
  // Pre-fill personal info from auth user after login
  ...
  const handleConfirmBooking = async () => {
    ...
    try {
      ...
      const { data: customerData, error: customerError } = await supabase
        ...
        .upsert(
          {
            id: user.id, // use auth user id as customer id
      ...
      if (customerError) {
        // If id conflict, try inserting with generated id
        ...
        const { error: bookingError } = await supabase.from('Booking').insert({
          ...
          tripDateId: `trip-date-${form.tourId}`,
          bookingType: 'direct',
        ...
        // 2. Insert booking with existing customer
        const { error: bookingError } = await supabase.from('Booking').insert({
          id: crypto.randomUUID(),
          reference: ref,
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 mt-10">
          ...
          <div className="lg:col-span-8">
            <div className="bg-card rounded-3xl border border-border p-7 md:p-10 shadow-card">
              {currentStep === 1 &&
              <Step1SelectTour form={form} selectTour={selectTour} />
              }
              {currentStep === 2 &&
              <Step2TravelDetails form={form} updateForm={updateForm} />
              }
              {currentStep === 3 &&
              <Step3PersonalInfo form={form} updateForm={updateForm} />
              }
              {currentStep === 4 &&
              <Step4Payment form={form} updateForm={updateForm} totalPrice={totalPrice} />
              }
    ...
    <div className="flex items-center justify-center">
      ...
      <React.Fragment key={step.id}>
          <div className="flex flex-col items-center gap-2">
            ...
            'completed' :
            currentStep === step.id ?
            'active' : 'border-border text-muted-foreground bg-white'}`
            }>
              {currentStep > step.id ?
            <Icon name="CheckIcon" size={16} /> :
            <span>{step.id}</span>
          ...
          {i < steps.length - 1 &&
        <div
          className={`step-connector flex-1 mx-2 mb-5 ${currentStep > step.id ? 'completed' : ''}`} />
        }
        </React.Fragment>
      )}
    </div>);
...
function Step1SelectTour({
  form,
  selectTour
}: {form: BookingFormData;selectTour: (t: typeof tourOptions[0]) => void;}) {
  ...
  return (
    <div>
      ...
      <p className="text-muted-foreground text-sm mb-7">
        Select the Kenya experience you'd like to book.
      </p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <button
          ...
          form.tourId === tour.id ?
          'border-accent bg-accent/5' : 'border-border hover:bg-muted/40'}`
          }>
            <div className="flex items-start justify-between gap-3 mb-3">
              <span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${catColor[tour.category]}`}>
                {tour.category}
              </span>
              {form.tourId === tour.id &&
...
function Step2TravelDetails({
  form,
  updateForm
}: {form: BookingFormData;updateForm: (k: keyof BookingFormData, v: string | number | null) => void;}) {
  ...
  return (
    <div>
      ...
      <p className="text-muted-foreground text-sm mb-7">
        Tell us when you're traveling and how many people are joining.
      </p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div>
          ...
          <div className="flex items-center gap-3">
            ...
            <span className="w-12 text-center font-display text-xl font-semibold text-foreground">
              {form.groupSize}
            </span>
            <button
              type="button"
        ...
        <div>
          ...
          <div className="px-4 py-3 rounded-xl bg-primary/5 border border-primary/20 flex items-baseline gap-2">
            <span className="font-display text-2xl font-semibold text-primary">
              ${form.tourPrice * form.groupSize}
            </span>
            <span className="text-sm text-muted-foreground">
              ({form.groupSize} × ${form.tourPrice})
            </span>
          </div>
        </div>
...
function Step4Payment({
  ...
  updateForm,
  totalPrice
}: {form: BookingFormData;updateForm: (k: keyof BookingFormData, v: string | number | null) => void;totalPrice: number;}) {
  return (
    <div>
      <h2 className="font-display text-2xl font-semibold text-foreground mb-2">
        Payment
      ...
      <p className="text-muted-foreground text-sm mb-7">
        Total due:{' '}
        <span className="font-display font-semibold text-primary text-lg">${totalPrice}</span>.
      ...
      <div className="p-6 rounded-2xl bg-green-50 border border-green-200">
          ...
          <label className="block text-xs font-semibold uppercase tracking-wider text-green-700 mb-2">
            M-Pesa Phone Number
          </label>
          <input
          type="tel"
      ...
      <div className="p-6 rounded-2xl bg-indigo-50 border border-indigo-200 text-center">
          ...
          <p className="text-sm text-indigo-700 leading-relaxed">
            You'll be redirected to PayPal to complete your secure payment of{' '}
            <strong>${totalPrice}</strong>. Your booking will be confirmed immediately after.
          </p>
        </div>
      }
      ...
      <div className="p-6 rounded-2xl bg-orange-50 border border-orange-200 text-center">
          ...
          <p className="text-sm text-orange-700 leading-relaxed">
            Accept payments across Africa — mobile money, bank cards, and bank transfers.
            You'll be redirected to complete payment of <strong>${totalPrice}</strong>.
          </p>
        </div>
      }
    </div>);
...
function BookingSummary({
  ...
  totalPrice,
  currentStep
}: {form: BookingFormData;totalPrice: number;currentStep: number;}) {
  return (
      ...
      <div className="bg-card rounded-3xl border border-border p-6 shadow-card">
        ...
        <>
            ...
            <div className="relative h-36 rounded-2xl overflow-hidden mb-5">
              ...
              <div className="absolute bottom-3 left-3">
                <span className="text-white text-xs font-semibold bg-accent px-2 py-0.5 rounded-full">
                  Selected Tour
                </span>
              </div>
            </div>

            <h4 className="font-display font-semibold text-foreground mb-1 leading-snug">
              {form.tourName}
            </h4>
          ...
          <div className="mt-4 flex items-center gap-2 p-3 rounded-xl bg-muted">
                ...
                <span className="text-xs font-medium text-foreground capitalize">
                  {form.paymentMethod === 'mpesa' ? 'M-Pesa STK Push' :
              form.paymentMethod === 'card' ? 'Credit/Debit Card' :
              form.paymentMethod === 'paypal' ? 'PayPal' : 'Flutterwave'}
                </span>
              </div>
          }
          </> :
        ...
        <div className="flex flex-col items-center py-8 text-center">
            ...
            <p className="text-sm text-muted-foreground">
              Select a tour to see your booking summary here.
            </p>
          </div>
        }
      </div>
...
function BookingSuccess({
  ...
  totalPrice,
  bookingRef
}: {form: BookingFormData;totalPrice: number;bookingRef: string;}) {
  return (
    <div className="min-h-screen bg-background flex items-center justify-center pt-24 pb-20 px-6">
      <div className="max-w-lg w-full text-center">
        ...
        <p className="text-muted-foreground mb-8 leading-relaxed">
          Your Kenya adventure is booked. We've sent a confirmation email to{' '}
        ...
        <div className="bg-card rounded-3xl border border-border p-7 text-left mb-8 shadow-card">
          ...
          <div className="space-y-3 text-sm">
            ...
            <div className="flex justify-between">
                ...
                <span className="font-medium text-foreground">{form.checkIn} → {form.checkOut}</span>
              </div>
            }
            <div className="flex justify-between">
              <span className="text-muted-foreground">Group Size</span>
              <span className="font-medium text-foreground">{form.groupSize} {form.groupSize === 1 ? 'person' : 'people'}</span>
            </div>
