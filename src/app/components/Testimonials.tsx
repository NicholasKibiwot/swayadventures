...
const FALLBACK_TESTIMONIALS: Testimonial[] = [
  {
    ...
    rating: 5,
    quote:
      "SwayAdventures made our Maasai Mara trip absolutely seamless. Our guide Joseph knew exactly where the lions were at sunrise — I've never felt so close to wild Africa. Worth every shilling.",
  ...
  },
  {
    ...
    rating: 5,
    quote:
      "Seeing Kilimanjaro at dawn with 300 elephants in the foreground was a moment I'll never forget. The camp was luxury, the food was incredible, and the team handled everything perfectly.",
  ...
  },
  {
    ...
    rating: 5,
    quote:
      "As a Kenyan, I thought I knew my own country — but SwayAdventures showed me Diani in a completely new light. The boutique hotel they chose was extraordinary. Already planning my next trip.",
...
export default function Testimonials() {
  ...
  /* ---------- LIVE DATA: read from the Testimonial table ---------- */
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const { data, error } = await supabase
          .from('Testimonial')
          .select('id, name, location, rating, message, tripName, avatar')
        ...
        setTestimonials(
          data.map((t: any) => ({
            ...
            name: t.name,
            location: t.location,
            tour: t.tripName || 'SwayAdventures Tour',
            rating: Number(t.rating) || 5,
            quote: t.message,
            image: t.avatar || '/assets/images/no_image.png',
            alt: `Portrait of ${t.name}`
          }))
        );
        setActive((a) => Math.min(a, Math.max(data.length - 1, 0)));
      } catch {
        /* keep fallback */
      }
    };
    ...
    const channel = supabase
      ...
      .on('postgres_changes', { event: '*', schema: 'public', table: 'Testimonial' }, () => load())
      .subscribe();
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {testimonials?.map((t, i) =>
            <div
              key={t?.id}
              onClick={() => setActive(i)}
              className={`scroll-reveal stagger-${Math.min(i + 2, 6)} cursor-pointer rounded-2xl p-7 border transition-all duration-400 ${
                active === i ?
